...
export async function fetchSharedNarration(backendUrl, categoriesKey, gender) {
  ...
  const url   = `${backendUrl}/api/narration`
    + `?date=${today}`
 ...
 */
export async function storeSharedNarration(backendUrl, categoriesKey, gender, narration, writeSecret = '') {
  ...
  try {
    await fetch(`${backendUrl}/api/narration`, {
      method:  'POST',
      headers,
      body:    JSON.stringify({ date: today, categories: categoriesKey, gender, narration }),
